...
unsigned long rightCaveStart = 0;
const unsigned long CAVE_CONFIRM_MS = 120;
...
void loop() {
  ...
  bool isRightCave = (C == BLACK && R == BLACK);

  if (isRightCave) {
    if (rightCaveStart == 0) rightCaveStart = millis();

    if (millis() - rightCaveStart >= CAVE_CONFIRM_MS) {
      rightCount++;                 
      rightCaveStart = 0;
